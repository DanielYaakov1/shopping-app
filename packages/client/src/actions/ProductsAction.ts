...
export const getAllProductsAction = async (pageNumber?: number, productPerPage?: number) => {
  try {
    const response = await fetch(`/api/v1/products?skip=${pageNumber}&limit=${productPerPage}`);
